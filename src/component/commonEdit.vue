...
<script lang="ts">
import { onMounted,defineComponent, onBeforeUnmount, ref, toRefs,onUpdated } from 'vue';
 ...
 // 设置富文本编辑器的HTML内容
  ...
  setup(props: any, content: any) {
    ...
    onMounted(() => {
      ...
      // 开启本地上传图片(这是后端上传链接)
      instance.config.uploadImgServer = '/api/updateImage/articles';
	  // 参数名字
      ...
      // 开启本地上传视频(这是后端上传链接)
      instance.config.uploadVideoServer = '/api/updateImage/articles';
      // 设置编辑器高度
      instance.config.height = 600;
      // 设置编辑器页面层级
      instance.config.zIndex = 10;
      // 设置编辑器placeholder
      instance.config.placeholder = '请输入您的描述！';
      // 配置编辑器显示颜色
	  ...
	  instance.config.uploadImgHooks = {
	  	success: (xhr:any, editor:any, result:any) => {
			console.log('插入图片内容', result.data.file_path)
			
		},
		customInsert: (insertImg:any, result:any, editor:any) => {
			console.log('插入图片内容', result)
